...
@APP.route('/resources')
def resources():
    quote_number = int(request.args.get('quote_number', 0))
    quote_number = quote_number % len(AFFIRMATIONS)
    quote = AFFIRMATIONS[quote_number]
...
def signup():
    ...
    if request.method == 'GET':
        ...
        add_user(name, username, password, age, language)
...
def messages():
    ...
    if 'username' in session:
        ...
        username = session['username']
        user = get_user_from_database(username)
        if username in ['abby', 'karen']:
            is_mentor = True
        else:
            is_mentor = False
        return render_template('messages.html', is_mentor=is_mentor, messages=get_all_messages(), user=user)

    return render_template('messages.html', is_mentor=False)
